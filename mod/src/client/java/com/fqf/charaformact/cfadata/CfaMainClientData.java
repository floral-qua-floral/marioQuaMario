...
//		this.PLAYER.cfa$getOldAnimationData().replaceAnimation(this, action.PIECEMEAL_ANIMATION, -1);
	...
	private static class WallInfoWithInputs extends DirectionBasedWallInfo {
		...
		protected WallInfoWithInputs(CfaPlayerData owner) {
			super(owner);
		}
...
//			CharaFormAct.LOGGER.info("World-space inputs: ({}, {}, {})", worldSpaceInputs.x, worldSpaceInputs.y, worldSpaceInputs.z);
		...
		private static class ClientButtonInput implements ButtonInput {
			...
			private void update(boolean isHeld, boolean isPressed) {
				if(isHeld && isPressed && !this.isHeld)
					this.pressBuffer = CharaFormAct.CONFIG.getBufferLength();
				else
					this.pressBuffer--;
				this.isHeld = isHeld;
			}
