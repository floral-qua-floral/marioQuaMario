...
public class CfaAnimationData {
	...
	public void replaceAnimation(CfaPlayerData data, PlayermodelAnimation newAnim, int ticksUntilAutoReplace) {
		...
		if(this.shouldResetAnimation((CfaAnimatingData) data)) {
			this.changingAnim = true;
			this.animationTicks = 0;
		}
		this.ticksUntilAutoReplaceAnimation = ticksUntilAutoReplace;
	}
	private boolean shouldResetAnimation(CfaAnimatingData data) {
	...
	private float calculateProgress(CfaPlayerData data) {
		ProgressHandler handler = this.currentAnim.progressHandler();
		if(handler == null) return 1;
		else return handler.calculator().calculateProgress((CfaAnimatingData) data, this.animationTicks);
	}
	private void conditionallyAnimateArm(
			Arrangement arrangement, LimbAnimation limbAnimation, CfaPlayerData data, float progress,
	...
	private void setupTailArrangement(
			Arrangement arrangement, CfaPlayerData data,
...
//		}
	...
	public void mutate(Arrangement arrangement, Arrangement.Mutator mutator, CfaPlayerData data, float progress) {
		...
		mutator.mutate((CfaAnimatingData) data, arrangement, progress);
		if(isMirrored) {
