...
public abstract class CfaMoveableData extends CfaPlayerData implements CfaTravelData {
	...
	private class CfaVelocities {
		...
		private CfaVelocities ensureDirty() {
			this.isDirty = true;
			return this.ensure();
		}
		private void apply() {
...
//		if(vertical > 0) this.getPlayer().fallDistance = 0;
	...
	public void goTo(Vec3d pos) {
		if(this.getPlayer() instanceof CfaPositionSettable mainClientPlayer) mainClientPlayer.cfa$setPos(pos);
		else if(this.getPlayer() instanceof ServerPlayerEntity serverPlayer) ((CfaPositionSettable) serverPlayer.networkHandler).cfa$setPos(pos);
	...
	@Override public void approachAngleAndAccel(
		...
		if (redirectDelta == 0 || (forwardAngleContribution == 0 && strafeAngleContribution == 0) ||
			...
			if (redirectDelta > 0) redirectedVel = CfaMoveableData.slerp(currentVel, intendedAngle, redirectDelta);
			else
