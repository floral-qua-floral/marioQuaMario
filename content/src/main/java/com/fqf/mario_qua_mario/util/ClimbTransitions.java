...
import com.fqf.mario_qua_mario.actions.wallbound.ClimbIntangibleDirectional;
import com.fqf.mario_qua_mario.actions.wallbound.ClimbWall;
...
public class ClimbTransitions {
	public static boolean canClimbBlock(BlockState state, Direction direction) {
		if(direction.getAxis().isHorizontal() && canClimbBlock(state)) {
			...
			if(state.isIn(MQMTags.SOMETIMES_CLIMBABLE_PANES))
	...
	private static boolean canClimbBlock(BlockState state) {
		return state.isIn(MQMTags.CLIMBABLE) && !state.isOf(Blocks.SCAFFOLDING);
	}
	...
	public static boolean isNonSolidClimbable(CfaReadableMotionData data, BlockPos pos, BlockState state, boolean directionality) {
	...
	public static boolean inNonSolidClimbable(CfaReadableMotionData data, boolean directionality) {
		return isNonSolidClimbable(data, data.getPlayer().getBlockPos(), data.getPlayer().getBlockStateAtPos(), directionality);
	}

	private static boolean tryingToStartClimbingIntangible(CfaReadableMotionData data) {
		if(data.getInputs().JUMP.isPressed()) return true;
		return MarioQuaMario.CONFIG.doAutoLadder() && data.getRecordedCollisions().collidedHorizontally();
	}

	private static BlockPos getClimbingBlockPos(CfaClientData data) {
		PlayerEntity mario = data.getPlayer();
	...
	public static void playGrabIntangibleSound(CfaClientData data, long seed) {
	...
	public static final CfaStat CLIMB_JUMP_UP_VEL = new CfaStat(1, StatCategory.JUMP_VELOCITY);
