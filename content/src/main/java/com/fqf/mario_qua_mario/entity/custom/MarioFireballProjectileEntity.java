	...
	public void tick() {
		...
		if(!this.getWorld().isClient() && ProjectileUtil.getCollision(this, this::canHit, RaycastContext.ShapeType.OUTLINE) instanceof BlockHitResult blockHitResult) {
			if(this.getWorld().getBlockState(blockHitResult.getBlockPos()).isIn(MQMTags.DESTROYED_BY_FIREBALL)) {
				...
				this.getWorld().playSound(null, blockHitResult.getBlockPos(), MarioSFX.BURN_OBJECT, SoundCategory.BLOCKS, 1, 1);
//				this.playSound(MarioSFX.BURN_OBJECT, 1, 1);
				this.discard();
			}
		}
	...
	private void bounce() {
		...
		if(!this.getWorld().isClient) this.playSound(MarioSFX.FIREBALL_WALL, 0.233F, 0.75F);
	}
	...
	protected void onBlockHit(BlockHitResult result) {
		...
		if(canBounce && (result.getSide().getAxis() == Direction.Axis.Y)) {
			...
			if(canBounce && this.getWorld().isSpaceEmpty(this, this.getBoundingBox().offset(checkAtOffset))) {
				...
				this.playSound(MarioSFX.FIREBALL_WALL, 0.45F, 1);
				this.discard();
			}
		}
	}
