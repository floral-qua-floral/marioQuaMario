...
public interface Bappable {
	 ...
	 */
	default BapResult cfa$getBapResult(
			CfaData data, World world,
			BlockPos pos, BlockState blockState, float hardness,
			Direction direction,
			int strength
	) {
		// Cannot affect indestructible blocks.
		if(hardness == -1 || blockState.isIn(CfaTags.UNBAPPABLE))
			return BapResult.FAIL;

		if(blockState.isIn(CfaTags.USES_DOUBLE_HARDNESS_WHEN_BAPPED))
			hardness *= 2;
		if(blockState.isIn(CfaTags.USES_HALF_HARDNESS_WHEN_BAPPED))
			hardness *= 0.5F;
		...
		boolean shouldPower = !blockState.isIn(CfaTags.NOT_POWERED_WHEN_BAPPED);
